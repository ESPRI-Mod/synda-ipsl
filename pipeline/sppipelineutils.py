...
def build_pipeline(name, transitions, state, add_args=True):
...
def decode_dataset_pattern(kw):
    ...
    assert kw.project.lower() == from_facets['project'].lower()
...
def get_args(self, kw):
    ...
    cfg = read_config('{0}/{1}.json'.format(spconfig.pipeline_folder, re.sub('_(VARIABLE|DATASET)', '', kw.pipeline)))
